...
output_title = 'Tech_Detection_XGBoost_Classifier_Result.csv'
...
while seed_index < len(seed_list):
    ...
    pred_col_name = 'Predict_{0}'.format(seed_index)
    pred_prob_col_name = 'Predict_Prob_{0}'.format(seed_index)
    ...
    df_50k_vec_output[pred_col_name] = best_model.predict(df_50k_vec)
    df_50k_vec_output[pred_prob_col_name] = best_model.predict_proba(df_50k_vec)[:, 1]
...
df_result.to_csv(output_title, index = False)
df_50k_vec_output.reset_index().to_csv('Original_ET50k_Prediction.csv', index = False)
